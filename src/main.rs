    ...
    if let Some(backend) = fix_backend {
        ...
        if fix_candidates.is_empty() {
            ...
            for (path, mediainfo) in fix_candidates {
                ...
                let sharing = match fix::sharing(path) {
                    fix::Sharing::Plain => None,
                    fix::Sharing::Symlink => Some(
                        "symlink, remuxing replaces it with a regular file so the target stays on disk"
                            .to_owned(),
                    ),
                    fix::Sharing::Hardlinked(links) => Some(format!(
                        "{links} hardlinks, remuxing unshares this copy so the original stays on disk"
                    )),
                };
                if let Some(warning) = sharing {
