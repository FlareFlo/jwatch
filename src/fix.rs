...
/// What else keeps a copy of this file once we remux it
#[derive(Debug, PartialEq)]
pub enum Sharing {
    Plain,
    /// The remux replaces the link itself, so the target is untouched and its own link
    /// count says nothing about the consequences
    Symlink,
    /// Shares an inode with other names, e.g. a torrent copy
    Hardlinked(u64),
}

/// Remuxing always writes a new inode, so anything else referencing the old one keeps it and
/// the library grows by a copy. Stat failures report Plain to stay quiet; a file we cannot
/// read fails loudly enough during the remux.
pub fn sharing(path: &Path) -> Sharing {
    use std::os::unix::fs::MetadataExt;
    if fs::symlink_metadata(path).is_ok_and(|m| m.file_type().is_symlink()) {
        return Sharing::Symlink;
    }
    match fs::metadata(path).map(|m| m.nlink()) {
        Ok(links) if links > 1 => Sharing::Hardlinked(links),
        _ => Sharing::Plain,
    }
}
...
mod tests {
    ...
    fn hardlinked_files_report_every_link() {
        ...
        assert_eq!(sharing(&a), Sharing::Hardlinked(2));
        assert_eq!(sharing(&b), Sharing::Hardlinked(2));
    }

    #[test]
    fn unshared_and_missing_files_are_plain() {
        let dir = tempfile::tempdir().unwrap();
        let plain = dir.path().join("plain.mkv");
        fs::write(&plain, b"x").unwrap();

        assert_eq!(sharing(&plain), Sharing::Plain);
        assert_eq!(
            sharing(&dir.path().join("does-not-exist.mkv")),
            Sharing::Plain
        );
    }

    /// Remuxing a symlink swaps the link, never the target, so the target's link count
    /// would describe a file we do not touch
    #[test]
    fn a_symlink_outranks_the_link_count_of_its_target() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("target.mkv");
        fs::write(&target, b"x").unwrap();
        fs::hard_link(&target, dir.path().join("twin.mkv")).unwrap();
        let link = dir.path().join("link.mkv");
        std::os::unix::fs::symlink(&target, &link).unwrap();

        assert_eq!(sharing(&target), Sharing::Hardlinked(2));
        assert_eq!(sharing(&link), Sharing::Symlink);
    }
